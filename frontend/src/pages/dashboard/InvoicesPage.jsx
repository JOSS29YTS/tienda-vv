...
const InvoicesPage = () => {
    ...
    const fetchPaymentMethods = async () => {
        try {
            ...
            if (res.ok) {
                ...
                setPaymentMethods(data.filter(m => !m.nb_metodo_pago.toLowerCase().includes('mixto')));
            }
        } catch (err) {
            console.error(err);
        }
    ...
    const handlePaymentSubmit = async () => {
        ...
        try {
            ...
            // Loop through payments and send each
            for (const p of payments) {
                if (!p.methodId || !p.amount) continue;

                // currency check
                const method = paymentMethods.find(m => m.id_metodo_pago == p.methodId);
                const isBs = method && !['USD', 'DIVISA', 'ZELLE'].some(k => method.nb_metodo_pago.toUpperCase().includes(k));
                ...
                const payload = {
                    id_factura_proveedor: selectedInvoice.id_factura_proveedor,
                    id_metodo_pago: p.methodId,
                    monto: finalAmount, // Always USD
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <header className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                <div>
                    <h2 className="text-3xl font-black text-slate-800 font-heading">Facturas Pendientes</h2>
                        ...
                        >
                            <div className="bg-slate-900 text-white p-6 flex justify-between items-center">
                                <div>
                                    ...
                                    <p className="text-slate-400 text-sm">Proveedor: {selectedInvoice.nb_proveedor}</p>
                                </div>
                            ...
                            <div className="p-6 max-h-[70vh] overflow-y-auto">
                                <div className="mb-6 bg-slate-50 p-4 rounded-xl border border-slate-200">
                                    <div className="flex justify-between items-center mb-1">
                                        ...
                                        <span className="font-mono font-black text-xl text-slate-800">$ {parseFloat(selectedInvoice.monto_restante).toFixed(2)}</span>
                                    </div>
                                    <div className="text-right text-xs font-bold text-slate-400">
                                        ≈ Bs {(parseFloat(selectedInvoice.monto_restante) * rate).toFixed(2)}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    {payments.map((p, idx) => (
                                        <div key={idx} className="flex gap-3 items-end">
                                            ...
                                            <div className="w-32">
                                                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Monto {
                                                    p.methodId && paymentMethods.find(m => m.id_metodo_pago == p.methodId)?.nb_metodo_pago.toUpperCase().includes('USD') ? '($)' : '(BS)'
                                                }</label>
                                                <input
                                                    type="number"
                                                    value={p.amount}
                                                    onChange={e => updatePaymentRow(idx, 'amount', e.target.value)}
                                                    className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 font-bold text-sm text-right"
                                                    placeholder="0.00"
                                                />
                                            </div>
                                            {idx > 0 && (
                                                <button onClick={() => removePaymentRow(idx)} className="p-2.5 text-red-400 hover:bg-red-50 rounded-lg">
                                                    <FaTimes />
                                                </button>
                                            )}
                                        </div>
                                    ...
                                    <button onClick={addPaymentRow} className="text-sm font-bold text-emerald-600 hover:text-emerald-700 flex items-center gap-1">
